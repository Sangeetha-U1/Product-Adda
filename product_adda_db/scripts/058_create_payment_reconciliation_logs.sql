...
(
    ...
    CONSTRAINT pk_reconciliation_logs_id
        PRIMARY KEY (pk_reconciliation_log_id),

    CONSTRAINT fk_reconciliation_logs_gateway_id
        FOREIGN KEY (fk_gateway_id)
        REFERENCES payment_gateways(pk_gateway_id)
        ON DELETE RESTRICT,

    CONSTRAINT fk_reconciliation_logs_status_id
        FOREIGN KEY (fk_status_id)
        REFERENCES payment_statuses(pk_status_id)
        ON DELETE RESTRICT,

    CONSTRAINT fk_reconciliation_logs_triggered_by
        FOREIGN KEY (fk_triggered_by_user_id)
...
CREATE INDEX idx_reconciliation_logs_gateway
ON payment_reconciliation_logs(fk_gateway_id);

CREATE INDEX idx_reconciliation_logs_status
ON payment_reconciliation_logs(fk_status_id);

CREATE INDEX idx_reconciliation_logs_run_id
ON payment_reconciliation_logs(reconciliation_run_id);

CREATE INDEX fk_reconciliation_logs_triggered_by
ON payment_reconciliation_logs(fk_triggered_by_user_id);
