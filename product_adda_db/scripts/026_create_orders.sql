...
Represents a single order placed by a customer, including payment references,
payment lifecycle timestamps, refund tracking, promotional discounts,
delivery information, and idempotency guarantees.
===============================================================================
*/
...
(
    ...
    fk_payment_id BINARY(16) NULL,

    payment_initiated_at_utc TIMESTAMP NULL,
    payment_confirmed_at_utc TIMESTAMP NULL,
    ...
    CONSTRAINT fk_orders_delivery_partner_id
    ...
    CONSTRAINT chk_orders_coupon_discount
    ...
    CONSTRAINT chk_orders_total_refunded
        CHECK (total_refunded_amount_in_paise >= 0)
);
...
CREATE INDEX idx_orders_cart
ON orders(fk_cart_id);
...
ON orders(fk_delivery_partner_id);

CREATE INDEX idx_orders_payment_id
ON orders(fk_payment_id);
