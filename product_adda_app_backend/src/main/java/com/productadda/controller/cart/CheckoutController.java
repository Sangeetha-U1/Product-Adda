...
public class CheckoutController {
    ...
    @Operation(summary = "Pre-validate the active cart before checkout",
            description = "Runs stock, coupon, and address checks without committing any data. "
    ...
    @Operation(summary = "Complete checkout and create an order",
            description = "Atomically validates the cart, calculates final totals, creates the order, "
                    + "converts reserved inventory, and marks the cart completed. "
                    + "Supports idempotent retry via the Idempotency-Key header.")
    ...
    public ResponseEntity<ApiSuccessResponseDto<CheckoutResponseDto>> checkout(
            @Parameter(description = "Client-generated UUID to guard against duplicate checkout submissions",
                    required = true)
            @RequestHeader(value = "Idempotency-Key") String idempotencyKey,
            @Valid @RequestBody CheckoutRequestDto requestDto) {

        CheckoutResponseDto response = checkoutService.checkout(requestDto, idempotencyKey);
