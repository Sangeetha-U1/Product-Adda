...
public class FinanceReconciliationReportInitiationService {

    private static final long MAX_DATE_RANGE_DAYS = 365;
    private static final String REPORT_TYPE_NAME = "FINANCE_RECONCILIATION";
    private static final String PENDING_STATUS_NAME = "PENDING";
    private static final String AUDIT_ACTION_NAME = "REPORT_GENERATED";
    private static final String DEFAULT_REPORT_DEPTH = "SUMMARY";
     ...
     * reconciliation report (admin access only). Access control note: no
     * dedicated FINANCE role exists yet — ADMIN/SUPER_ADMIN is used per the
     * Day 0 decision; a // TODO is left below for a future dedicated role.
     * ================================================================
     */
    @Transactional
    public ReportJobStatusDto initiateReconciliationReport(ReportRequestDto request) {
        ...
        String reportDepth = request.getReportDepth() == null || request.getReportDepth().trim().isEmpty()
                ? DEFAULT_REPORT_DEPTH
                : request.getReportDepth();
        if (!"SUMMARY".equals(reportDepth) && !"DETAILED".equals(reportDepth)) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "report_depth must be SUMMARY or DETAILED");
        }
        ...
        if (!isAdmin) {
            throw new ApiException(HttpStatus.FORBIDDEN, "Only admins can generate finance reconciliation reports");
        }

        ReportType reportType = reportTypeRepository.findByReportTypeName(REPORT_TYPE_NAME)
                .orElseThrow(() -> new ApiException(HttpStatus.INTERNAL_SERVER_ERROR,
                        "FINANCE_RECONCILIATION report type not configured"));
        ...
        filters.put("reportDepth", reportDepth);
        ...
        Report reportJob = Report.builder()
                ...
                .reportName("Finance Reconciliation Report " + now.toLocalDate())
                .filters(filters)
                .retryCount(0)
                .generatedAtUtc(now)
                .isActive(true)
