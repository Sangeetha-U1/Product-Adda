...
public class AdminSalesReportInitiationService {

    private static final long MAX_DATE_RANGE_DAYS = 365;
    private static final String REPORT_TYPE_NAME = "SALES";
     ...
     */
    @Transactional
    public ReportJobStatusDto initiateSalesReport(ReportRequestDto request) {
        ...
        if (!isAdmin) {
            throw new ApiException(HttpStatus.FORBIDDEN, "Only admins can generate platform reports");
        }

        ReportType reportType = reportTypeRepository.findByReportTypeName(REPORT_TYPE_NAME)
                .orElseThrow(() -> new ApiException(HttpStatus.INTERNAL_SERVER_ERROR,
                        "SALES report type not configured"));
        ...
        Report reportJob = Report.builder()
                ...
                .reportName("Admin Sales Report " + now.toLocalDate())
                .filters(filters)
                .retryCount(0)
                .generatedAtUtc(now)
                .isActive(true)
