...
public class VendorPerformanceReportInitiationService {

    private static final long MAX_DATE_RANGE_DAYS = 365;
    private static final String REPORT_TYPE_NAME = "VENDOR_PERFORMANCE";
     ...
     * 5. RESPONSE MAPPING
     * ================================================================
     * Description: Creates a PENDING async job record for a vendor
     * performance report. Admins may request an unrestricted or
     * multi-vendor view; vendors are hard-scoped server-side to their own
     * vendor id, never trusted from the request body.
     * ================================================================
     */
    @Transactional
    public ReportJobStatusDto initiateVendorPerformanceReport(ReportRequestDto request) {
        ...
        if (!isAdmin && !isVendor) {
            throw new ApiException(HttpStatus.FORBIDDEN,
                    "Only admins and vendors can generate vendor performance reports");
        }
        ...
        ReportType reportType = reportTypeRepository.findByReportTypeName(REPORT_TYPE_NAME)
                .orElseThrow(() -> new ApiException(HttpStatus.INTERNAL_SERVER_ERROR,
                        "VENDOR_PERFORMANCE report type not configured"));
        ...
        Report reportJob = Report.builder()
                ...
                .reportName("Vendor Performance Report " + now.toLocalDate())
                .filters(filters)
                .retryCount(0)
                .generatedAtUtc(now)
                .isActive(true)
        ...
        // sales_aggregates grouped by vendor_id, calculate net_earnings =
        // total_gmv - commissions, and join payout status per vendor.
