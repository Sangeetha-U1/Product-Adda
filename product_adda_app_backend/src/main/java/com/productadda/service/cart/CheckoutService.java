    ...
    public CheckoutResponseDto checkout(CheckoutRequestDto requestDto, String idempotencyKeyHeader) {
        ...
        if (existingOrder != null) {
            ...
            if (!existingOrder.getFkUser().getPkUserId().equals(user.getPkUserId())) {
                throw new ApiException(HttpStatus.FORBIDDEN, "Idempotency key does not belong to this user");
            }
        ...
        if (!address.getFkUser().getPkUserId().equals(user.getPkUserId())) {
            throw new ApiException(HttpStatus.FORBIDDEN, "Address does not belong to the authenticated user");
        }
        ...
        // Stock check re-verified independently (Zero-Trust: never assumes /validate ran first).
        ...
        Order savedOrder = orderCreationService.createOrder(
                user, cart, cartItems, address, coupon, pendingStatus, totals, idempotencyKey);
        ...
        // Inventory conversion: permanently deduct ordered quantity from available stock.
        ...
        // Deactivate (soft) the add-to-cart inventory reservations now that stock has been converted.
