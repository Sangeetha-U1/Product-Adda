...
public interface OrderRepository extends JpaRepository<Order, UUID> {
        ...
        // ==========================================
        // DELIVERY PARTNER AVAILABLE ORDERS RETRIEVAL
        // Description: Fetches a pageable list of active, unclaimed SHIPPED orders
        // where payment status is SUCCESS, ordered by oldest first.
        // ==========================================
        @Query("SELECT o FROM Order o " +
                        "LEFT JOIN FETCH o.fkStatus " +
                        "LEFT JOIN FETCH o.fkAddress " +
                        "WHERE o.isActive = true " +
                        "AND o.fkDeliveryPartner IS NULL " +
                        "AND o.fkStatus.statusName = 'SHIPPED' " +
                        "AND EXISTS (SELECT p FROM Payment p WHERE p.fkOrder = o AND p.fkStatus.statusName = 'SUCCESS' AND p.isActive = true) "
                        +
